...
/*   Updated: 2025/09/24 20:03:17 by tbhuiyan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void fill(char **tab, t_point size, char target, int pos_y, int pos_x)
{
	if (pos_y < 0 || pos_x < 0 || pos_y >= size.y || pos_x >= size.x) // on check si les valeurs entree sont coherente
		return;
	if (tab[pos_y][pos_x] == 'F' || tab[pos_y][pos_x] != target) // on check si la case est deja visite ou que la case de depart n'est pas la target
		return;
